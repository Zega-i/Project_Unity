...
const MaterialsScreen = () => {
  ...
  useFocusEffect(useCallback(() => { loadReadIds(); }, []));
  ...
  const filtered = materials.filter(m => {
    ...
    if (search.trim()) {
      ...
      return m.title.toLowerCase().includes(q) || m.subject.toLowerCase().includes(q) || m.teacher.toLowerCase().includes(q);
    }
    return true;
  });
  ...
  const unreadCount = materials.filter(m => !readIds.includes(m.id)).length;
        ...
        {subjects.length > 0 ? subjects.map(subject => (
          <View key={subject} style={styles.section}>
            ...
            <View style={styles.sectionHeader}>
              ...
              <Text style={[styles.sectionCount, { color: colors.textSecondary }]}>{grouped[subject].length} materi</Text>
            </View>
            ...
            <View style={styles.list}>
              {grouped[subject].map(item => {
                ...
                return (
                  <Pressable
                    key={item.id}
                    style={[styles.card, { backgroundColor: colors.card, borderColor: isRead ? colors.border : PURPLE + '40' }]}
                  ...
                  >
                    ...
                    <View style={styles.cardBody}>
                      <View style={styles.cardTopRow}>
                        ...
                        {!isRead && <Text style={[styles.unreadLabel, { color: PURPLE }]}>Belum Dibaca</Text>}
                      </View>
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyDesc, { color: colors.textSecondary }]}>
              {search ? 'Coba kata kunci lain atau ubah filter' : 'Belum ada materi yang diupload guru'}
            </Text>
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  unreadBadgeWrap: { width: 40, alignItems: 'flex-end' },
  ...
  searchWrap: { flexDirection: 'row', alignItems: 'center', gap: 10, marginHorizontal: 20, marginTop: 14, marginBottom: 4, borderRadius: 14, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 10 },
  ...
  list: { gap: 10 },
  card: { flexDirection: 'row', alignItems: 'center', borderRadius: 18, padding: 14, borderWidth: 1.5, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.04, shadowRadius: 8, elevation: 1 },
  iconBox: { width: 52, height: 52, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 12, position: 'relative' },
  iconText: { fontSize: 22 },
  unreadDot: { position: 'absolute', top: 4, right: 4, width: 8, height: 8, borderRadius: 4, backgroundColor: PURPLE },
